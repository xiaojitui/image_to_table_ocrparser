...
def find_table_structure(allwords, page_positions, table_edges, min_sep = 25):
    ...
    ele_n_1 = [k for k in ele_n if 1<k<=8]
    if ele_n_1 != []:
        col_n = mode(ele_n_1)[0][0]
    else:
        ele_n_2 = [k for k in ele_n if 8<k<=12]
        if ele_n_2 != []:
            col_n = mode(ele_n_2)[0][0]
        else:
            col_n = 4
    
    #col_n = min(8, max(ele_n))
...
def preparedf_img_single(allwords, page, W, col_seperators):
    ...
    for i in range(1, len(picked)):
        ...
        if picked[i]['row'] == picked[i-1]['row'] and picked[i]['col'] <= picked[i-1]['col']:
            ...
            if 2<=i<= len(picked)-2 and picked[i-1]['col'] == picked[i-2]['col'] + 1 \
                and picked[i]['col']+1 == picked[i+1]['col']:
                picked[i]['text'] = picked[i-1]['text'] + ' ' + picked[i]['text']
